...
def accuracy(output, target, topk=(1,)):
    ...
    return res
def adjust_learning_rate(optimizer, epoch, init_lr, decay):
    lr = init_lr * (decay ** (epoch // 30))
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
    return lr

def train_model(experiment, model, dataloaders, criterion, optimizer, earlyStopping, init_lr, decay):
    ...
    while (StopCond):
        ...
        lr = adjust_learning_rate(optimizer, epoch, init_lr, decay)
...
model_ft = SqueezeNet(num_classes=num_classes,dropout=0.5)
...
model_ft, hist5x, hist1x = train_model(experiment, model_ft, dataloaders, criterion, optimizer_ft, early_stopping, learningRate, decay)