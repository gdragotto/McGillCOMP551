...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, squeeze_planes,
        ...
        self.res1 = Fire(inplanes, squeeze_planes,expand1x1_planes, expand3x3_planes)
...
class SqueezeNet(nn.Module):
    ...
    def __init__(self, version='original', num_classes=1000, dropout=0.5):
        ...
        self.features = nn.Sequential(
                ...
                Fire(64, 16, 64, 64),
                BasicBlock(128, 16, 64, 64),
                nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
                Fire(128, 32, 128, 128),
                BasicBlock(256, 32, 128, 128),
                nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
                Fire(256, 48, 192, 192),
                BasicBlock(384, 48, 192, 192),
                Fire(384, 64, 256, 256),
                BasicBlock(512, 64, 256, 256),
            )
...
model_ft = SqueezeNet(num_classes=num_classes,dropout=0.25)
if torch.cuda.device_count() > 1:
    print("Let's use", torch.cuda.device_count(), "GPUs!")
    model_ft = nn.DataParallel(model_ft)
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
model_ft = model_ft.to(device)
optimizer_ft = optim.SGD(model_ft.parameters(), lr=learningRate, momentum=0.95, weight_decay=weight_decay)
