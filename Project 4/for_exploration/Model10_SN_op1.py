...
def train_model(experiment, model, dataloaders, criterion, optimizer, earlyStopping=5):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                if (prec5 == best_acc_5x and prec1 >best_acc_1x) or (prec5 >= best_acc_5x):
                        ...
                        torch.save(bestModel, "Model_" + unique + "__x5_" + str(round(prec5,4)) + "_x1_" + str(round(prec1,4)) +".pt")
...
experiment = Experiment(api_key="<HIDDEN>",project_name="squeezenet-adj",workspace="evelynyang94")
data_dir = "/home/yang94/project/yang94/courses/COMP551/miniproject4/tiny-imagenet-200/"
...
model_ft = SqueezeNet(num_classes=num_classes)
...
optimizer_ft = optim.Adam(model_ft.parameters(), lr=learningRate, weight_decay=weight_decay, eps=0.1)
...
model_ft, hist5x, hist1x = train_model(experiment, model_ft, dataloaders, criterion, optimizer_ft, earlyStopping=early_stopping)